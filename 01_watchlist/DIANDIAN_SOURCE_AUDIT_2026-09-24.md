...
- 点点月度日期选择器已实际设置并确认 `2021-09` 至 `2026-08`。
- 默认表格为每页 50 条；“总额”占一条，因此第一页只显示 49 个月。表格底部同时显示 `共 61 条` 与第 2 页。
- 将分页切换为 `100条/页` 后，Township 与 Hay Day 的下载、收入均显示完整 60 个月。第一页 49 个月不是账号权限上限。
- 新产品按点点首个可见月或已核验上线月截断；My Perfect Hotel 下载从 2022-04 起，收入从 2022-08 起。
- Heartopia 全球下载页与区域合集收入页不是同 scope；两条序列只分别分析，不计算跨 scope RPD。
